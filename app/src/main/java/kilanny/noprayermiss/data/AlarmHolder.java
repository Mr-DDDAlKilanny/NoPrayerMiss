...
import kilanny.noprayermiss.util.Utils;
...
public class AlarmHolder extends RecyclerView.ViewHolder implements PopupMenu.OnMenuItemClickListener {
    ...
    public void bind() {
        ...
        if (diff.dayOfMonth().get() > 1)
            ...
            mtxtTime.setText(itemView.getContext().getString(R.string.afterNHours, diff.hourOfDay().get()));
        else {
            int total = diff.hourOfDay().get() * 60 + diff.minuteOfDay().get();
            mtxtTime.setText(itemView.getContext().getString(R.string.afterNMinutes, total));
        }
        // days
